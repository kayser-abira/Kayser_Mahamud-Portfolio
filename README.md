...
<head>
  ...
  <meta name="description" content="Portfolio of Md. Kayser Mahamud — projects, skills, and contact." />
  <title>Md. Kayser Mahamud | Portfolio</title>

  <!-- Favicon (optional) -->
  <link rel="icon" href="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'%3E%3Ccircle cx='50' cy='50' r='48' fill='%230ea5e9'/%3E%3Ctext x='50' y='60' text-anchor='middle' font-size='52' font-family='Arial' fill='white'%3EM%3C/text%3E%3C/svg%3E" />
  ...
  <style>
    ...
    body {
      ...
      color: var(--text);
      background: radial-gradient(1200px 600px at 70% -10%, rgba(34,211,238,0.15), transparent 60%),
                  radial-gradient(900px 400px at -10% 20%, rgba(167,139,250,0.14), transparent 50%),
                  var(--bg);
      line-height: 1.6;
    }
    ...
    <section class="hero">
      <div class="container wrap">
        ...
        <div class="hero-card card">
          <h3 style="margin:6px 0 4px;">Quick Facts</h3>
          <div class="stat">
...
🌱 Always learning, growing, and solving problems to enhance productivity through tech-driven ERP solutions.
  ...
  <script>
    ...
    const onScroll = () => {
      ...
      for (let i = sections.length - 1; i >= 0; i--) {
        ...
        if (s && s.offsetTop <= y) {
          ...
          const id = '#' + s.id; const active = document.querySelector(`.nav a[href='${id}']`);
    ...
    // ------------------------------
    // Reveal on intersect + animate skill bars
    // ------------------------------
    const io = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          entry.target.classList.add('visible');
          // Animate bars within visible section
          entry.target.querySelectorAll('.bar > span').forEach(span => {
            const val = span.getAttribute('data-val') || 0;
